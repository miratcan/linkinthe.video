...
if modal:
    ...
    @app.function(image=image, secrets=[modal.Secret.from_name("linkinthe-video")])  # type: ignore[attr-defined]
    ...
    def run_pipeline(video_id: int):  # pragma: no cover - fallback
        raise RuntimeError("Modal is not installed; cannot run remote pipeline.")
