...
class OpenAIProvider(LLMProvider):
    """litellm-backed OpenAI provider."""
    ...
    def extract_products(self, transcript: str) -> List[Dict[str, Any]]:
        try:
            ...
            raise RuntimeError(
                "litellm is required for OpenAIProvider"
            ) from exc

        prompt = (
            "Extract product mentions with optional timestamps from this "
            "transcript. Return JSON list objects with keys: name, timestamp "
            "(MM:SS), asin (if obvious).\n"
            f"Transcript:\n{transcript}"
        )
